 ...
 *
 * Role-gated server-side (issue #27): the session role must be `admin` —
 * the jobs surface is ops-only per the nav registry (lib/auth/nav.ts).
 * Unauthenticated requests redirect to /login via the (app) group layout;
 * authenticated non-admins get a 404. The interim
 * `JOBS_OBSERVABILITY_TOKEN` gate is retired now that iron-session login
 * (architecture.md §5) is the chokepoint.
...
import { requireOneOf } from "@/lib/auth/guard";
...
export default async function JobsPage() {
  // Server-side role gate (issue #27): admin only — 404 for other roles.
  await requireOneOf(["admin"]);
